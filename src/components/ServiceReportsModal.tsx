...
export default function ServiceReportsModal({
  ...
  vehicles.forEach(v => {
    ...
    unifiedItems.push({
      ...
      phone: v.kontak,
      details: `Armada: ${v.kendaraan} | Tujuan: ${v.tujuan}${v.tanggal ? ` | Tanggal: ${v.tanggal} (${v.jam || v.waktu || ''})` : ''}`,
      dateStr: v.tanggal ? `${v.tanggal}${v.jam || v.waktu ? ` ${v.jam || v.waktu}` : ''}` : (v.createdAt ? new Date(v.createdAt).toLocaleDateString('id-ID') : 'Aktif'),
      status: v.status,
      statusCategory: cat,
